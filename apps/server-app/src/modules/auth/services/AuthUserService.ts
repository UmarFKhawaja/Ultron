import { ProviderType, User, UserService } from '@ultron/core-library';
...
export class AuthUserService {
  constructor(
    @inject(MYSQL_CONSTANTS.Symbols.Services.UserService)
  ...
  async ensureUserWithProvider(displayName: string, userName: string, emailAddress: string, providerType: ProviderType, providerID: string): Promise<User> {
    ...
    user = await this.userService.linkUserToProvider(user.id, providerType, providerID);

    return user;
  }

  async ensureUserNotWithProvider(user: User, providerType: ProviderType): Promise<User> {
    user = await this.userService.unlinkUserFromProvider(user.id, providerType);

    return user;
  }

  async updateUser(user: User, displayName: string, userName: string): Promise<User> {
    user = await this.userService.updateUser(user.id, displayName, userName, user.emailAddress);
  ...
  async resetPassword(user: User): Promise<User> {
    user = await this.userService.unsetPassword(user.id);
  ...
  async changeEmailAddress(user: User, emailAddress: string): Promise<User> {
    user = await this.userService.updateUser(user.id, user.displayName, user.userName, emailAddress);
  ...
  async changePassword(user: User, oldPassword: string, newPassword: string): Promise<User> {
    ...
    user = await this.userService.setPassword(user.id, hashSync(newPassword));
